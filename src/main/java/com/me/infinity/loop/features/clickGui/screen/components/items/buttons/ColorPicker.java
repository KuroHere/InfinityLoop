...
public class ColorPicker extends Button {
    ...
    public ColorPicker(Setting colorSetting) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        FontRender.drawString3(colorSetting.getName(),(int) this.x, (int) this.y + 2,new Color(red,green,blue).getRGB());
        RenderUtil.draw2DGradientRect(this.x, this.y + 12, this.x + 91, this.y + 52, Color.getHSBColor(hsb[0], 0f, 0f).getRGB(), Color.getHSBColor(hsb[0], 0f, 1f).getRGB(), Color.getHSBColor(hsb[0], 1f, 0f).getRGB(), Color.getHSBColor(hsb[0], 1f, 1f).getRGB());



        float steps = 8.8f;
        for (float i = 0.0F; i + steps <= 96.8; i += steps) {
            RenderUtil.draw1DGradientRect(this.x + i, this.y + 57, this.x + 3 + steps + i, this.y + 62, Color.getHSBColor(i / 88, 1f, 1f).getRGB(), Color.getHSBColor((i + steps) / 88, 1f, 1f).getRGB());
        }
        ...
        RenderUtil.draw2DGradientRect(this.x, this.y + 70 , this.x + 91, this.y + 75, minAlpha, minAlpha, maxAlpha, maxAlpha);
        ...
        int indicatorHue = (int) ((this.x) + (int) (hsb[0] * 88));
...
//        RenderUtil.drawRect2(this.x,this.y,this.x + 94,this.y + 90,new Color(0xB3232323, true).getRGB());
    ...
    private boolean handleMouseClick(int mouseX, int mouseY, int mouseButton) {
        ...
        if (mouseWithinBounds(mouseX, mouseY,this.x , this.y + 12f, 91f, 40f)) {
            getRGBfromClick(mouseX, mouseY);
        } else if (mouseWithinBounds(mouseX, mouseY, this.x , this.y + 57, 88, 5)) {
            getHueFromClick(mouseX);
        } else if (mouseWithinBounds(mouseX, mouseY,this.x , this.y + 70 , 88, 5)) {
