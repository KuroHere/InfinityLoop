...
public class ColorButton extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        if (ClickGui.getInstance().sideSettings.getValue().booleanValue()) {
            ...
            RenderUtil.drawRect(this.x, this.y, this.x + 0.7f, this.y + (float) this.height + 1.0f, sideColor);
        }
        try {
        ...
        RenderUtil.drawOutlineRect(x + width - 5, y + 2, x + width + 7, y + height - 2, new Color(0xFFFFFF), 0.4f);
        ...
        if (setting.isOpen) {
            ...
            InfinityLoop.textManager.drawStringWithShadow(isInsideCopy(mouseX, mouseY) ? ChatFormatting.UNDERLINE + "Copy" : "Copy", x + 25f, y + 113, -1);
            InfinityLoop.textManager.drawStringWithShadow(isInsidePaste(mouseX, mouseY) ? ChatFormatting.UNDERLINE + "Paste" : "Paste", x + 60f, y + 113, -1);
    ...
    public boolean isInsideCopy(int mouseX, int mouseY) {
        return mouseOver((int) x + 25, (int) y + 113, (int) (x + 25) + InfinityLoop.textManager.getStringWidth("Copy"), (int) (y + 112) + InfinityLoop.textManager.getFontHeight(), mouseX, mouseY);
    }

    public boolean isInsidePaste(int mouseX, int mouseY) {
        return mouseOver((int) x + 60, (int) y + 113, (int) (x + 60) + InfinityLoop.textManager.getStringWidth("Paste"), (int) (y + 112) + InfinityLoop.textManager.getFontHeight(), mouseX, mouseY);
    }
    ...
    public void drawPicker(Setting setting, int pickerX, int pickerY, int hueSliderX, int hueSliderY, int alphaSliderX, int alphaSliderY, int mouseX, int mouseY) {
        ...
        RenderUtil.drawOutlineRect(cursorX - 2, cursorY - 2, cursorX - 2, cursorY - 2, new Color(0xFFFFFF), 0.4f); //cursorX - 2, cursorY - 2, cursorX + 2, cursorY + 2
    ...
    void drawHueSlider(int x, int y, int width, int height, float hue) {
        ...
        if (height > width) {
            ...
            RenderUtil.drawOutlineRect(x, sliderMinY - 1, x + width, sliderMinY + 1, new Color(0xFFFFFF), 0.4f);
        } else {
            for (int colorIndex = 0; colorIndex < 6; colorIndex++) {
                int previousStep = Color.HSBtoRGB((float) step / 6, 1.0f, 1.0f);
                int nextStep = Color.HSBtoRGB((float) (step + 1) / 6, 1.0f, 1.0f);
            ...
            RenderUtil.drawOutlineRect(sliderMinX - 1, y, sliderMinX + 1, y + height, new Color(0xFFFFFF), 0.4f);
        }
    }
