...
import me.loop.api.events.impl.player.EventEntityMove;
import me.loop.api.events.impl.render.PreRenderEvent;
...
import me.loop.api.utils.impl.maths.AstolfoAnimation;
...
public class PlayerTrails extends Module {
    ...
    public Setting<Float> down = add(new Setting("Down", 0.5F, 0.0F, 2.0F));
    public Setting<Float> width = add(new Setting("Height", 1.3F, 0.1F, 2.0F));
    private  final Setting<Boolean> shfix =this.add( new Setting<>("ShaderFix", false));

    public Setting<modeEn> mode = add(new Setting<>("ColorMode", modeEn.Ukraine));
    private final Setting<Color> color = this.add(new Setting<>("Color", new Color(0x8800FF00)));
    Map<EntityPlayer, List<Trail> > entAndTrail = new HashMap<>();

    public PlayerTrails() {
        super("PlayerTrails", "Render Line Behind U", Category.RENDER, true, false, false);
    }
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        for (EntityPlayer entity : mc.world.playerEntities) {
            ...
            mc.entityRenderer.updateCameraAndRender(mc.getRenderPartialTicks(), 2);
