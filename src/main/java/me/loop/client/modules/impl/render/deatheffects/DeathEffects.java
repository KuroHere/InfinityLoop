package me.loop.client.modules.impl.render.deatheffects;
...
public class DeathEffects extends Module {
    private final Setting<Boolean> sound = this.add(new Setting<>("Sound", false));
    private final Setting<Boolean> particles = this.add(new Setting("Particles", false));
    private final Setting<Moderator> mode = this.add(new Setting("Mode", Moderator.SMOKE));
    private final Setting<Float> yOffset = this.add(new Setting("YPos offset", 0.0, 0.0, 2.0, v -> this.particles.getValue().booleanValue()));

    public DeathEffects() {
        super("DeathEffects", "Spawn a thunder to the loser", Category.RENDER, true, false, false);
    ...
    public void onDeath(EventDeath event) {
        if (!Client.nullCheck() && event.player != null) {
