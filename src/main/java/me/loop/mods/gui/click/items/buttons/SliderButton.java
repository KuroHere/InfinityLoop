...
public class SliderButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (Colors.getInstance().rainbow.getValue().booleanValue()) {
            int color = ColorUtil.changeAlpha(HUD.getInstance().colorMap.get(MathUtil.clamp((int) this.y, 0, this.renderer.scaledHeight)), InfinityLoop.moduleManager.getModuleByClass(ClickGui.class).moduleEnableC.getValue().getAlpha());
            int color1 = ColorUtil.changeAlpha(HUD.getInstance().colorMap.get(MathUtil.clamp((int) this.y + this.height, 0, this.renderer.scaledHeight)), InfinityLoop.moduleManager.getModuleByClass(ClickGui.class).moduleEnableC.getValue().getAlpha());
            ...
            if (ClickGui.INSTANCE.sliderType.getValue() == ClickEnum.SliderType.Line) {
                ...
                RenderUtil.drawRect(x, y, ((Number) setting.getValue()).floatValue() <= min.floatValue() ? x : getWidth(), y + (float) height - 0.5f, !isHovering(mouseX, mouseY) ? InfinityLoop.colorManager.getColorWithAlpha(120) : InfinityLoop.colorManager.getColorWithAlpha(200));
            }
            RenderUtil.drawLine(x + 1, y, x + 1, y + (float) height - 0.5f, 0.9f, InfinityLoop.colorManager.getColorWithAlpha(255));
        ...
        InfinityLoop.textManager.drawStringWithShadow(this.getName() + " \u00a77" + (this.setting.getValue() instanceof Float ? (Number) ((Number) this.setting.getValue()) : (Number) ((Number) this.setting.getValue()).doubleValue()), this.x + 2.3f, this.y - 1.7f - (float) InfinityLoopGui.INSTANCE.getTextOffset(), -1);
    }
