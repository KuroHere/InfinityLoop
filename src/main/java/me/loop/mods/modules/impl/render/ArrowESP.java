...
public class ArrowESP
        extends Module {
    private final Setting<Integer> red = this.add(new Setting<Integer>("Red", 255, 0, 255));
    private final Setting<Integer> green = this.add(new Setting<Integer>("Green", 0, 0, 255));
    private final Setting<Integer> blue = this.add(new Setting<Integer>("Blue", 255, 0, 255));
    private final Setting<Integer> radius = this.add(new Setting<Integer>("Placement", 45, 10, 200));
    private final Setting<Float> size = this.add(new Setting<Float>("Size", Float.valueOf(10.0f), Float.valueOf(5.0f), Float.valueOf(25.0f)));
    private final Setting<Boolean> outline = this.add(new Setting<Boolean>("Outline", true));
    private final Setting<Float> outlineWidth = this.add(new Setting<Float>("Outline-Width", Float.valueOf(1.0f), Float.valueOf(0.1f), Float.valueOf(3.0f)));
    private final Setting<Integer> fadeDistance = this.add(new Setting<Integer>("Range", 100, 10, 200));
    private final Setting<Boolean> invisibles = this.add(new Setting<Boolean>("Invisibles", false));
    private final EntityListener entityListener = new EntityListener();

    public ArrowESP() {
        super("ArrowESP", "Arrow tracers ", Category.RENDER, true, false);
    }
    ...
    public static double interp(double d, double d2) {
        return d2 + (d - d2) * (double) Util.mc.getRenderPartialTicks();
    }
