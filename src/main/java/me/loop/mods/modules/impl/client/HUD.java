...
import me.loop.mods.modules.impl.misc.ToolTips;
        ...
        if (this.arrayList.getValue()) {
            if (this.renderingUp.getValue()) {
                ...
                for (int k = 0; k < (this.alphabeticalSorting.getValue() ? Managers.moduleManager.alphabeticallySortedModules.size() : Managers.moduleManager.sortedModules.size()); ++k) {
                    ...
                    final int n = height;
                    j += 10;
                    this.renderer.drawString(text5, x, (float) (n - j), ClickGui.INSTANCE.colorSync.getValue() && (Colors.getInstance()).rainbow.getValue().booleanValue() ? (((Colors.getInstance()).rainbowModeA.getValue() == Colors.rainbowModeArray.Up) ? ColorUtil.rainbow(counter1[0] * (Colors.getInstance()).rainbowHue.getValue().intValue()).getRGB() : ColorUtil.rainbow((Colors.getInstance()).rainbowHue.getValue().intValue()).getRGB()) : this.color, true);
                    counter1[0] = counter1[0] + 1;
                }
            }
        }
