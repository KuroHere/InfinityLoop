package me.loop.features.modules.combat;

import me.loop.InfinityLoop;
import me.loop.event.events.UpdateWalkingPlayerEvent;
import me.loop.features.modules.Module;
import me.loop.features.setting.Setting;
import me.loop.util.impl.*;
import me.loop.util.init.Timer;
...
public class Killaura extends Module {
    ...
    private void doKillaura() {
        if (this.onlySharp.getValue().booleanValue() && !EntityUtil.holdingWeapon(Util.mc.player)) {
            target = null;
            return;
        }
        int wait = !this.delay.getValue().booleanValue() ? 0 : (int) (DamageUtil.getCooldownByWeapon(Util.mc.player) * (this.tps.getValue().booleanValue() ? InfinityLoop.serverManager.getTpsFactor() : 1.0F));
    ...
    private Entity getTarget() {
        ...
        for (Entity entity : Util.mc.world.playerEntities) {
            if (((!this.players.getValue().booleanValue() || !(entity instanceof EntityPlayer)) && (!this.animals.getValue().booleanValue() || !EntityUtil.isPassive(entity)) && (!this.mobs.getValue().booleanValue() || !EntityUtil.isMobAggressive(entity)) && (!this.vehicles.getValue().booleanValue() || !EntityUtil.isVehicle(entity)) && (!this.projectiles.getValue().booleanValue() || !EntityUtil.isProjectile(entity))) || (entity instanceof net.minecraft.entity.EntityLivingBase &&
                ...
                continue;
            if (!Util.mc.player.canEntityBeSeen(entity) && !EntityUtil.canEntityFeetBeSeen(entity) && Util.mc.player.getDistanceSq(entity) > MathUtil.square(this.raytrace.getValue().floatValue()))
                continue;
            if (target == null) {
                target = entity;
