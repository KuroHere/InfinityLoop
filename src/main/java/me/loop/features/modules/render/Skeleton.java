package me.loop.features.modules.render;

import me.loop.InfinityLoop;
import me.loop.event.events.Render3DEvent;
...
import me.loop.util.impl.BlockUtil;
...
public class Skeleton
    ...
    private void drawSkeleton(Render3DEvent event, EntityPlayer e) {
        ...
        if (entPos != null && e.isEntityAlive() && !e.isDead && e != Skeleton.mc.player && !e.isPlayerSleeping()) {
            ...
            if (InfinityLoop.friendManager.isFriend(e.getName())) {
                GlStateManager.color(0.0f, 191.0f, 230.0f, (float) this.alpha.getValue().intValue());
            } else {
                GlStateManager.color((float) Colors.getInstance().c.getValue().getRed() / 255.0f, (float) Colors.getInstance().c.getValue().getGreen() / 255.0f, (float) Colors.getInstance().c.getValue().getBlue() / 255.0f, (float) this.alpha.getValue().intValue());
            }
