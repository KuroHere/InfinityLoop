...
import me.loop.features.modules.client.clickgui.ClickEnum;
...
public class SliderButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (ClickGui.getInstance().colorSync.getValue() && Colors.getInstance().rainbow.getValue().booleanValue()) {
        ...
        InfinityLoop.textManager.drawStringWithShadow(this.getName() + " \u00a77" + (this.setting.getValue() instanceof Float ? (Number)((Number)this.setting.getValue()) : (Number)((Number)this.setting.getValue()).doubleValue()), this.x + 2.3f, this.y - 1.7f - (float)InfinityLoopGui.getClickGui().getTextOffset(), -1);
    }
    ...
    public boolean isHovering(int mouseX, int mouseY) {
        for (Component component : InfinityLoopGui.getInstance().getComponents()) {
