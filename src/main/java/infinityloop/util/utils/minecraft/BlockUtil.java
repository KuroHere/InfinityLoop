...
import infinityloop.util.utils.RotationUtil;
...
public class BlockUtil
    ...
    public static boolean isPositionPlaceable(BlockPos pos, boolean rayTrace) {
    ...
    public static boolean isPositionPlaceable(final BlockPos position, final boolean entityCheck, final boolean sideCheck) {
        ...
        if (entityCheck) {
            for (final Object entity : BlockUtil.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(position))) {
    ...
    public static boolean isPositionPlaceable(final BlockPos pos, final boolean entityCheck, final double distance) {
        ...
        if (entityCheck) {
            for (final Object entity : BlockUtil.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(pos))) {
                if (BlockUtil.mc.player.getDistance((Entity) entity) > distance) {
                    continue;
                }
                if (entity instanceof EntityItem) {
                    continue;
    ...
    public static void clickBlock(final BlockPos position, final EnumFacing side, final EnumHand hand, final boolean packet) {
        if (packet) {
            BlockUtil.mc.player.connection.sendPacket((Packet)new CPacketPlayerTryUseItemOnBlock(position.offset(side), side.getOpposite(), hand, Float.intBitsToFloat(Float.floatToIntBits(17.735476f) ^ 0x7E8DE241), Float.intBitsToFloat(Float.floatToIntBits(26.882437f) ^ 0x7ED70F3B), Float.intBitsToFloat(Float.floatToIntBits(3.0780227f) ^ 0x7F44FE53)));
        }
        else {
