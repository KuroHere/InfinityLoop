package infinityloop.features.gui.components;
...
import infinityloop.features.gui.components.items.buttons.Button;
...
import infinityloop.util.utils.maths.ImageUtil;
...
import static infinityloop.features.modules.client.ClickGui.ClickGui.getInstance;
...
public abstract class Component
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (Colors.getInstance().isEnabled()); {
            int n = color = (Colors.getInstance()).rainbow.getValue().booleanValue() ? (((Colors.getInstance()).rainbowModeA.getValue() == Colors.rainbowModeArray.Up) ? ColorUtil.rainbow((Colors.getInstance()).rainbowHue.getValue().intValue()).getRGB() : ColorUtil.rainbow((Colors.getInstance()).rainbowHue.getValue().intValue(), ClickGui.getInstance().hoverAlpha.getValue()).getRGB()) : ColorUtil.toARGB(getInstance().red.getValue(), getInstance().green.getValue(), getInstance().blue.getValue(), getInstance().alpha.getValue());
        }
        ...
        InfinityLoop.textManager.drawString(this.getName(), (float)this.x + 3.0f, (float)this.y - 4.0f - (float) InfinityLoopGui.getClickGui().getTextOffset(), -1);
        ...
        if (getInstance().moduleiconmode.getValue()) {
