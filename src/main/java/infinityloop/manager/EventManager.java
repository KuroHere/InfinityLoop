...
import infinityloop.event.events.network.ConnectionEvent;
import infinityloop.event.events.network.EventDeath;
import infinityloop.event.events.TotemPopEvent;
import infinityloop.event.events.network.EventPacket;
import infinityloop.event.events.player.EventMotionUpdate;
...
import infinityloop.features.modules.misc.PopCounter;
import infinityloop.util.utils.renders.GLUProjection;
...
public class EventManager extends Feature {
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        if (event.isCanceled())
            return;
        mc.profiler.startSection("infinityloop");
